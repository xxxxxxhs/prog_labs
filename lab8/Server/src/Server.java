...
public class Server {
    ...
    private void handlePacket(DatagramPacket packet) {
        ...
        processPool.submit(() -> {
            ...
            if (!response.isContainCollection() || response.getCollection().size() < 300) {
                sendResponse(response, packet.getAddress(), packet.getPort());
            }
            else {
                LinkedList<Response> responses = Response.split(response.getCollection());
                for (Response resp : responses) {
                    System.out.println("sent collection size: " + resp.getCollection().size());
                    sendResponse(resp, packet.getAddress(), packet.getPort());
                    try {
                        Thread.sleep(10);  // Искусственная задержка в 10 миллисекунд
