...
public class SqlDumper{
    ...
    private int addPerson(Person person) throws SQLException {
        ...
        String checkSql = "SELECT id FROM person WHERE name = ? AND passport_id = ? AND " +
                "eyecolor_id = (SELECT id FROM color WHERE value = ?) AND " +
                "haircolor_id = (SELECT id FROM color WHERE value = ?) AND " +
                "location_id = ?";
        try (PreparedStatement checkStmt = connection.prepareStatement(checkSql)) {
            ...
            if (checkRs.next()) {
                return checkRs.getInt("id"); // Возвращаем id, если координаты найдены
            }
        }
    ...
    private int addLocation(Location location) throws SQLException {
        String checkSql = "SELECT id FROM location WHERE name = ? AND x = ? AND y = ? AND z = ?";
        try (PreparedStatement checkStmt = connection.prepareStatement(checkSql)) {
            checkStmt.setString(1, location.getName());
            checkStmt.setDouble(2, location.getX());
            checkStmt.setFloat(3, location.getY());
            checkStmt.setLong(4, location.getZ());
            ResultSet checkRs = checkStmt.executeQuery();
            if (checkRs.next()) {
                return checkRs.getInt("id"); // Возвращаем id, если координаты найдены
            }
    ...
    public void update(Movie movie, long id) {
        String sql = "UPDATE movie SET " +
                "name = ?, coordinates_id = (SELECT id FROM coordinates WHERE x = ? and y = ? LIMIT 1), " +
                "creation_date = now(), oscars_count = ?, golden_palm_count = ?, " +
                "total_box_office = ?, mpaa_rating_id = (SELECT id FROM mpaa_rating WHERE value = ?), " +
                "person_id = (SELECT id FROM person WHERE name = ? AND passport_id = ? " +
                "AND eyecolor_id = (SELECT id FROM color WHERE value = ?)" +
                "AND haircolor_id = (SELECT id FROM color WHERE value = ?)" +
                "AND location_id = " +
                "(SELECT id FROM location WHERE x = ? AND y = ? AND z = ? AND name = ? LIMIT 1) LIMIT 1) " +
                "WHERE id = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql);) {
            pstmt.setString(1, movie.getName());
            ...
            pstmt.setString(9, movie.getScreenWriter().getPassportId());
            pstmt.setString(10, movie.getScreenWriter().getEyeColor().name());
            pstmt.setString(11, movie.getScreenWriter().getHairColor().name());
            pstmt.setDouble(12, movie.getScreenWriter().getLocation().getX());
            pstmt.setFloat(13, movie.getScreenWriter().getLocation().getY());
            pstmt.setLong(14, movie.getScreenWriter().getLocation().getZ());
            pstmt.setString(15, movie.getScreenWriter().getLocation().getName());
            pstmt.setLong(16, id);
            try {
